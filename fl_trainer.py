...
def get_results_filename(poison_type, attack_method, model_replacement, project_frequency, defense_method, norm_bound,
    ...
    elif defense_method in ("multi-metrics", "krum", "multi-krum", "rfa"):
...
def calc_norm_diff(gs_model, vanilla_model, epoch, fl_round, mode="bad"):
    ...
    if mode == "bad":
        ...
        logger.info(
            "===> ND `|w_normal-w_g|` in local epoch: {} | FL round: {} |, is {}".format(epoch, fl_round, norm_diff))
    elif mode == "avg":
        logger.info(
...
def train(model, device, train_loader, optimizer, epoch, log_interval, criterion, pgd_attack=False, eps=5e-4,
          model_original=None,
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if not pgd_attack:
            ...
            if proj == "l_inf":
                ...
                if (batch_idx % project_frequency == 0 or batch_idx == len(train_loader) - 1) and (
...
def test(model, device, test_loader, test_batch_size, criterion, mode="raw-task", dataset="cifar10",
    ...
    if mode == "raw-task":
        ...
        if dataset in ("mnist", "emnist"):
            ...
            logger.info('#### Targetted Accuracy of %5s : %.2f %%' % (
            classes[target_class], 100 * class_correct[target_class] / class_total[target_class]))
...
class FrequencyFederatedLearningTrainer(FederatedLearningTrainer):
    def __init__(self, arguments=None, *args, **kwargs):
        ...
        if arguments["defense_technique"] == "no-defense":
            ...
            self._defender = WeightDiffClippingDefense(norm_bound=arguments['norm_bound'])
        elif arguments["defense_technique"] == "multi-metrics":
            self._defender = Multi_metrics(num_workers=self.part_nets_per_round, num_adv=1)
    ...
    def run(self):
        ...
        for flr in range(1, self.fl_round + 1):
            ...
            if flr in self.attacking_fl_rounds:
                ...
                selected_node_indices = np.random.choice(self.num_nets, size=self.part_nets_per_round - 1,
                                                         replace=False)
                ...
                logger.info("FL round: {}, total num data points: {}, num dps poisoned: {}".format(flr, num_data_points,
                                                                                                   self.num_dps_poisoned_dataset))

                net_freq = [self.num_dps_poisoned_dataset / total_num_dps_per_round] + [
                    num_data_points[i] / total_num_dps_per_round for i in range(self.part_nets_per_round - 1)]
                ...
                for net_idx, net in enumerate(net_list):
                    ...
                    if net_idx == 0:
                        ...
                        if self.attack_case == "edge-case":
                            train_dl_local, _ = get_dataloader(self.dataset, './data', self.batch_size,
                                                               ...
                                                               dataidxs)  # also get the data loader
                    ...
                    adv_optimizer = optim.SGD(net.parameters(), lr=self.adv_lr * self.args_gamma ** (flr - 1),
                                              momentum=0.9,
                    ...
                    if net_idx == 0:
                        if self.prox_attack:
                            ...
                            for inner_epoch in range(1, self.local_training_period + 1):
                                estimate_wg(wg_clone, self.device, self.clean_train_loader, prox_optimizer, inner_epoch,
                                            log_interval=self.log_interval, criterion=self.criterion)
                            wg_hat = wg_clone

                        for e in range(1, self.adversarial_local_training_period + 1):
                            # we always assume net index 0 is adversary
                            if self.defense_technique in ('krum', 'multi-krum', 'multi-metrics'):
                                train(net, self.device, self.poisoned_emnist_train_loader, optimizer, e,
                                      log_interval=self.log_interval, criterion=self.criterion,
                                      pgd_attack=self.pgd_attack, eps=self.eps * self.args_gamma ** (flr - 1),
                                      model_original=model_original, project_frequency=self.project_frequency,
                                      adv_optimizer=adv_optimizer,
                                      prox_attack=self.prox_attack, wg_hat=wg_hat)
                            else:
                                train(net, self.device, self.poisoned_emnist_train_loader, optimizer, e,
                                      log_interval=self.log_interval, criterion=self.criterion,
                                      pgd_attack=self.pgd_attack, eps=self.eps, model_original=model_original,
                                      project_frequency=self.project_frequency, adv_optimizer=adv_optimizer,
                                      prox_attack=self.prox_attack, wg_hat=wg_hat)

                        test(net, self.device, self.vanilla_emnist_test_loader, test_batch_size=self.test_batch_size,
                             criterion=self.criterion, mode="raw-task", dataset=self.dataset,
                             poison_type=self.poison_type)
                        test(net, self.device, self.targetted_task_test_loader, test_batch_size=self.test_batch_size,
                             criterion=self.criterion, mode="targetted-task", dataset=self.dataset,
                             poison_type=self.poison_type)
                        ...
                        if self.model_replacement:
                            ...
                            for idx, param in enumerate(net.parameters()):
                                param.data = (param.data - model_original[idx]) * (
                                            total_num_dps_per_round / self.num_dps_poisoned_dataset) + model_original[
                                                 idx]
                            v = torch.nn.utils.parameters_to_vector(net.parameters())
                        ...
                        adv_norm_diff = calc_norm_diff(gs_model=net, vanilla_model=self.net_avg, epoch=e, fl_round=flr,
                                                       mode="bad")
                        ...
                        for e in range(1, self.local_training_period + 1):
                            train(net, self.device, train_dl_local, optimizer, e, log_interval=self.log_interval,
                                  criterion=self.criterion)
                            # at here we can check the distance between w_normal and w_g i.e. `\|w_bad - w_g\|_2`
                        # we can print the norm diff out for debugging
                        honest_norm_diff = calc_norm_diff(gs_model=net, vanilla_model=self.net_avg, epoch=e,
                                                          fl_round=flr, mode="normal")
                ...
                for net_idx, net in enumerate(net_list):
                    ...
                    honest_norm_diff = calc_norm_diff(gs_model=net, vanilla_model=self.net_avg, epoch=e, fl_round=flr,
                                                      mode="normal")
            ...
            if self.defense_technique == "no-defense":
            ...
            elif self.defense_technique == "multi-metrics":
                ...
                net_list, net_freq, epoch_choice, global_choice = self._defender.exec(client_models=net_list,
                                                                                      num_dps=[
                                                                                                  self.num_dps_poisoned_dataset] + num_data_points,
                                                                                      g_user_indices=g_user_indices,
                                                                                      device=self.device,
                                                                                      attack_round=attack_round)
                ...
                net_list, net_freq = self._defender.exec(client_models=net_list, net_freq=net_freq,
                                                         names=selected_node_indices, device=self.device)
            else:
                NotImplementedError("Unsupported defense method !")
            ...
            overall_acc, raw_acc = test(self.net_avg, self.device, self.vanilla_emnist_test_loader,
                                        test_batch_size=self.test_batch_size, criterion=self.criterion, mode="raw-task",
                                        dataset=self.dataset, poison_type=self.poison_type)
            backdoor_acc, _ = test(self.net_avg, self.device, self.targetted_task_test_loader,
                                   test_batch_size=self.test_batch_size, criterion=self.criterion,
        ...
        if self.poison_type == 'ardis':
            ...
            df1 = pd.DataFrame(
                {'fl_iter': [0], 'main_task_acc': [88], 'backdoor_acc': [11], 'raw_task_acc': [0], 'adv_norm_diff': [0],
                 'wg_norm': [0]})
            df = pd.concat([df1, df])

        results_filename = get_results_filename(self.poison_type, self.attack_method, self.model_replacement,
                                                self.project_frequency,
                                                self.defense_technique, self.norm_bound, self.prox_attack, False,
                                                self.model)
...
class FixedPoolFederatedLearningTrainer(FederatedLearningTrainer):
    def __init__(self, arguments=None, *args, **kwargs):
        ...
        self.attacker_pool = np.random.choice(self.num_nets, self.attacker_pool_size, replace=False)
        # arguments["attacker_pool"] = self.attacker_pool

    def run(self, arguments=None, *args, **kwargs):
        main_task_acc = []
        raw_task_acc = []
        backdoor_task_acc = []
        fl_iter_list = []
        ...
        for flr in range(1, self.fl_round + 1):
            ...
            selected_honest_users = [idx for idx in selected_node_indices if idx not in self.attacker_pool]
            logger.info("Selected Attackers in FL iteration-{}: {}".format(flr, selected_attackers))
            arguments["selected_honest_users"] = selected_honest_users
            ...
            if self.defense_technique == "no-defense":
                ...
                net_list, net_freq = self._defender.exec(client_models=net_list, names=selected_node_indices,
                                                         device=self.device)
            else:
                NotImplementedError("Unsupported defense method !")
