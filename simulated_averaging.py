...
if __name__ == "__main__":
    ...
    parser.add_argument('--defense_method', type=str, default="no-defense",
                        help='defense method used: no-defense|norm-clipping|norm-clipping-adaptive|weak-dp|multi-metrics|krum|multi-krum|rfa|')
    ...
    parser.add_argument('--selected_attackers', type=np.array, default=None,
                        help='indices of attackers in each round for fixed_pool FL')
    parser.add_argument('--selected_honest_users', type=np.array, default=None,
                        help='indices of honest clients in each round for fixed_pool FL')
    ...
    if READ_CKPT:
        if args.model == "lenet":
            ...
            with open("./checkpoint/Cifar10_VGG9_10epoch.pt".format(args.model.upper()), "rb") as ckpt_file:
                ckpt_state_dict = torch.load(ckpt_file, map_location=device)
    ...
    if args.fl_mode == "fixed-freq":
        ...
        print("argeument", arguments["net_avg"].parameters())
        fixed_pool_fl_trainer = FixedPoolFederatedLearningTrainer(arguments=arguments)
        fixed_pool_fl_trainer.run(arguments=arguments)
